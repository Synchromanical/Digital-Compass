...
/*
 * Add this structure so that the magnetometer code builds properly.
 * These fields correspond to the user’s code usage:
 *   - Temperature_Sensor
 *   - MagOutput_DataRate
 *   - MagFull_Scale
 *   - Working_Mode
...
typedef struct
{
  uint8_t Temperature_Sensor;    /*!< TEMP enable/disable bits             */
  uint8_t MagOutput_DataRate;    /*!< Magnetometer ODR (e.g. 0.75 Hz to 220 Hz) */
  uint8_t MagFull_Scale;         /*!< Magnetometer full-scale setting      */
  uint8_t Working_Mode;          /*!< Continuous, Single, or Sleep mode    */
} LSM303AGRMag_InitTypeDef;
...
/* USER PATCH BEGIN --------------------------------------------------------- */
/* Macros that did not exist in the original driver, but your code uses: */
#define LSM303AGR_DATA_READY_INTERRUPT_DISABLE   0x00
#define LSM303AGR_INTERRUPT_SIGNAL_LOW           0x00
#define LSM303AGR_INTERRUPT_REQUEST_PULSED       0x00
#define LSM303AGR_DEC_NO                         0x00
#define LSM303AGR_FIFO_DISABLE                   0x00

/*
 * Your code references these two types that are not part of the
 * original ST driver. We define them here so the compiler is happy.
 */
typedef struct
{
...
} LSM303AGR_AccITConfigTypeDef;

/*
 * A helper function to accept your LSM303AGRAcc_InitTypeDef struct,
 * assemble the bits, and then call the original LSM303AGR_AccInit(…).
 */
void LSM303AGR_AccInitStruct(LSM303AGRAcc_InitTypeDef *pAccInitStruct);

/*
 * A dummy function for configuring interrupts, so your code
 * that calls LSM303AGR_AccITConfig(&accITConfig) will compile.
 */
